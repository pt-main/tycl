...
- **`env("VAR", "default", "type")`**  
  Получает значение переменной окружения. Если переменная не задана или пуста, используется значение по умолчанию. Третий аргумент задаёт ожидаемый тип (`string`, `int`, `bool`, `float`) — результат будет приведён к этому типу.
...
- **`asString(value)`**  
  Преобразует переданное значение (объект, число, булево значение) в строку. Обычно используется для отладки или для создания строковых представлений сложных структур.
...
- **`get("path", "type")`**  
  Извлекает значение из любой части конфига (синтаксис пути - `[основной конфиг].[вложенный объект].[...]`, массивы не поддерживаются) по точечному пути (например, `"database.host"`) и приводит его к указанному типу. Путь может проходить через вложенные объекты. Это позволяет переиспользовать значения в разных частях конфига.
